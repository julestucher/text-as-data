...
I had some findings from applying LDA in my last blog post. There were a few discernable categories, which were associated with looking for housing, advertizing events happening, and people looking for people to meet up with and talk to. Thus far, I have only applied text methods to the post-based corpus and have not explored the comments or user profiles. In my initial blog post, I expressed interested in answering the research question: "How are local queer and LGBT people self-identifying on the alternative social media platform Lex?". I am still interested in investigating this question, so I have begun to clean and create a text corpus from the user profile data. With this data set, each document represents the text found on a user's Lex profile, subset to the relevant sections: About Me, pronouns, gender identify, sexuality, and relationship style. I then applied an LDA model to the user profile corpus, restricting to words that appeared at least 10 times and were found in at least 5% of profiles. LDA created 10 categories that included the expected sexual/relationship style preferences, gender and sexuality identiy labels, but also included dis/ability status and hobbies. In the top words from each category below, we can see some distinct groups of profiles: users who emphasize romantic/sexual interests (V1), being a student (V2), having autism/ADHD (V5), interest in horror/movies (V7), and their other hobbies unrelated to gender/sexuality (V8). Some top words that may not be considered mainstream LGBT identity words include: t4t (an orientation that stands for trans people interested in other trans people) and transmasc (trans men and non-binary people who are masculine of center).

![user_lda](/img/user_lda.png)
...
The below confusion matrix from the Naive Bayes model shows that `community` posts are most often misclassified as `event` or `friends` posts, `dating` posts are most often misclassified as `friends` posts, and `hookup` posts are more often misclassified as `dating` or `friends` posts than correctly classied. 

![nb_conf_matrix_multi](/img/nb_conf_matrix_multi.png)
